...
const AvdelingDashboard = ({ userDepartment }: { userDepartment?: string } = {}) => {
  const dept = userDepartment || 'KRS';
  ...
  const loadData = async () => {
    try {
      ...
      // Fetch department goals (doc ID is the department name, e.g., "KRS")
      try {
        const goalsDocRef = doc(db, 'allente_targets', dept);
      ...
      // Fetch all employees from department
      const employeesRef = collection(db, 'employees');
      const employeesQuery = query(employeesRef, where('department', '==', dept));
      ...
      console.error('Error loading avdeling data:', err);
      setLoading(false);
    }
  };
  ...
  return (
    <div className="avdeling-dashboard">
      <div className="avdeling-header">
        <h2>Avdeling: {dept}</h2>
        <p className="avdeling-subtitle">Se alle kontrakter fra {dept}</p>
      </div>
...
export default AvdelingDashboard;
