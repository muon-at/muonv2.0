...
export default function Chat() {
  ...
  // DM listener moved to LeftChatSidebar.tsx - runs globally, not just on Chat page
  ...
  const checkChannelAccess = (type: string, avdeling?: string, allowedUsers?: string[], project?: string): boolean => {
    ...
    switch (type) {
      case 'project':
        ...
        // Allente channel: only Allente employees + MUON owners
        if (channelProject === 'Allente') {
          return userProject === 'Allente' || user?.role === 'owner';
        }
          ...
          {selectedChannel || selectedDM ? (
            <>
              ...
              {selectedChannel && channels.find(c => c.id === selectedChannel) && (
                <div style={{
                  padding: '1rem 1.5rem',
                  background: selectedChannel === 'project-allente' ? '#1B1F2E' : '#667eea',
                  color: 'white',
                  borderBottom: '1px solid #e2e8f0',
                  display: 'flex',
                  justifyContent: 'center',
